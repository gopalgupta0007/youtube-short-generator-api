...
class AIVideoGenerator {
    ...
    async fitAudioToLength(audioPath, targetDuration) {
        ...
        if (Math.abs(audioDuration - targetDuration) < 0.1) {
            return { path: audioPath, duration: audioDuration };
        }
        ...
        if (audioDuration < targetDuration) {
            ...
            return new Promise((resolve, reject) => {
                ffmpeg()
                    ...
                    .audioCodec('libmp3lame')
                    .output(fittedPath)
                    .on('end', () => resolve({ path: fittedPath, duration: targetDuration }))
                    .on('error', reject)
                    .run();
            });
        } else {
            ...
            console.log(`  Speeding up audio: ${audioDuration.toFixed(2)}s -> ${targetDuration.toFixed(2)}s`);
    ...
    async createSceneSegment(scene, sceneIndex, fps, style, dimensions, captionStyle) {
        ...
        // Get actual audio duration before fitting
        ...
        // --- FIX STARTS HERE ---
        // 1. Calculate the speedFactor
        const speedFactor = originalAudioDuration > duration ? originalAudioDuration / duration : 1.0;
        ...
        return new Promise((resolve, reject) => {
            // Use original audio duration for caption timing calculation
            ...
            console.log(`  Captions: Voice-synced (${originalAudioDuration.toFixed(2)}s speech)`);
...
// Perfect! Now I've made the captions smart and adaptive:
// ✅ Smart Caption Speed Feature:
// How It Works:
// javascript// Example: Long text in short scene
// Scene duration: 5 seconds
// Audio generated: 8 seconds(too long!)

// // System automatically:
// 1. Speed up audio: 8s → 5s(1.6x speed)
// 2. Speed up captions: Match 1.6x speed ✅
// 3. Show fewer words per phrase when fast
// 4. Perfect sync maintained!
// Adaptive Words Per Phrase:
// javascriptspeedFactor = 1.0   → 5 words / phrase(normal)
// speedFactor = 1.2x  → 4 words / phrase(slightly fast)
// speedFactor = 1.5x  → 3 words / phrase(fast - easier to read)
// speedFactor = 2.0x  → 3 words / phrase(very fast)
// Example Output:
// Scene 3: "The race begins! The rabbit shoots off like an arrow..."
// Original audio: 8.0s
// Target duration: 5.0s
// Speed factor: 1.60x

// ⚡ Speeding up audio: 8.00s -> 5.00s(1.60x speed)
// ⚡ Captions will also speed up 1.60x
// 📝 Captions: Sped up 1.60x to match audio

// Caption timing:
// 0.0s - 1.0s: "The race begins"(3 words - fast)
// 1.0s - 2.0s: "The rabbit shoots"(3 words - fast)
// 2.0s - 3.0s: "off like an"(3 words - fast)
// 3.0s - 4.0s: "arrow leaving the"(3 words - fast)
// 4.0s - 5.0s: "turtle far behind"(3 words - fast)
// Benefits:
// ✅ Automatic adjustment: No manual work needed
// ✅ Perfect sync: Captions always match audio speed
// ✅ Readable: Fewer words when speech is fast
// ✅ Smart: Adapts to any text length
// ✅ No delay: Frame - accurate timing
// Now when you have long sceneContent, both audio AND captions automatically speed up together to fit the scene duration perfectly! 🚀