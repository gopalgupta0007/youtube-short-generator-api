...
class AIVideoGenerator {
     ...
     */
    splitTextIntoWords(text, audioDuration) {
        const cleanText = text.replace(/\\n/g, ' ').replace(/\n/g, ' ').replace(/\s+/g, ' ').trim();
     ...
     */
    buildWordByWordCaptions(text, audioDuration, styleName, dimensions) {
        const captionStyle = this.getCaptionStyle(styleName, dimensions);
        const wordTimings = this.splitTextIntoWords(text, audioDuration);
        
        // Group words into phrases (3-5 words at a time for readability)
     ...
     */
    buildVoiceSyncedCaptionFilters(text, audioDuration, styleName, dimensions) {
        const captionStyle = this.getCaptionStyle(styleName, dimensions);
        const phrases = this.buildWordByWordCaptions(text, audioDuration, styleName, dimensions);
