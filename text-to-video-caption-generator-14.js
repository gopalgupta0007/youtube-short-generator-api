...
const IMAGE_PROVIDERS = [
    ...
    },
    {
        ...
        // Community GPU network — works without any key, just slower queue
        generate: async (prompt, width, height, seed) => {
    ...
    {
        name: 'Craiyon',
        // Completely free, no key, returns base64 images
...
// ─── Stable Horde: Community GPU (anonymous, no key) ─────────────────────────
async function generateStableHorde(prompt, width, height, seed) {
    // Snap dimensions to multiples of 64 (SD requirement)
...
// ─── Craiyon: Completely free, no key ────────────────────────────────────────
...
class AIVideoGenerator {
    constructor() {
        ...
        // Lower resolution used for API calls, then upscaled with sharp
        this.genResolutions = {
    ...
    // ── IMAGE GENERATION: waterfall through all free providers ───────────────
    async generateImage(prompt, style, seed, dimensions, aspectRatio = '9:16') {
        ...
        console.log('    ⚠️  All providers failed — gradient placeholder');
    ...
    async generateVoiceover(text, outputPath, voice = 'Brian') {
        ...
        console.log(`🎙️  TTS: "${clean.substring(0, 50)}..."`);

        try {
            const r = await axios.get(this.ttsApis.streamElements, { params: { voice, text: clean }, responseType: 'arraybuffer', timeout: 30000 });
            fs.writeFileSync(outputPath, Buffer.from(r.data));
            console.log('✓ StreamElements TTS'); return outputPath;
        } catch { }
        try {
            const r = await axios.get(this.ttsApis.googleTTS, { params: { ie: 'UTF-8', q: clean, tl: 'en', client: 'tw-ob' }, responseType: 'arraybuffer', timeout: 30000, headers: { 'User-Agent': 'Mozilla/5.0' } });
            fs.writeFileSync(outputPath, Buffer.from(r.data));
            console.log('✓ Google TTS'); return outputPath;
        } catch { }
        try { await sayExport(clean, null, null, outputPath); console.log('✓ Local TTS'); return outputPath; } catch { }

        const est = Math.max(3, clean.split(' ').length * 0.4);
        console.log('⚠️  Silent fallback');
        return this.createSilentAudio(outputPath, est);
    }

    createSilentAudio(outputPath, duration) {
        return new Promise((resolve, reject) => {
            ffmpeg().input('anullsrc=r=44100:cl=stereo').inputFormat('lavfi').duration(duration)
    ...
    async fitAudioToLength(audioPath, target) {
        ...
        if (Math.abs(dur - target) < 0.1) return { path: audioPath, duration: dur, speedFactor: 1.0, originalDuration: dur };

        if (dur < target) {
            return new Promise((resolve, reject) => {
                ffmpeg().input(audioPath).input('anullsrc=r=44100:cl=stereo').inputFormat('lavfi')
        ...
        const sf = dur / target;
        return new Promise((resolve, reject) => {
            ffmpeg().input(audioPath).audioFilters([`atempo=${Math.min(sf, 2.0).toFixed(3)}`])
    ...
    async createSceneSegment(scene, idx, fps, style, dimensions, captionStyle, aspectRatio) {
        ...
        const captions = this.buildVoiceSyncedCaptionFilters(scene.sceneContent, origDur, fitted.speedFactor, captionStyle, dimensions);
        const pattern = path.join(this.framesDir, `scene${idx}_frame_%05d.png`);

        // return new Promise((resolve, reject) => {
        //     ffmpeg()
        //         .input(pattern).inputOptions(['-framerate', String(fps), '-t', String(duration)])
        //         .input(fitted.path)
        //         .outputOptions(['-c:v','libx264','-pix_fmt','yuv420p','-r',String(fps),'-crf','20','-preset','medium','-c:a','aac','-b:a','192k','-t',String(duration)])
        //         .videoFilters(captions).output(segmentPath)
        //         .on('progress', p => { if (p.percent) process.stdout.write(`\r  Scene ${idx+1}: ${p.percent.toFixed(1)}%`); })
        //         .on('end', () => { console.log(`\n  ✅ Scene ${idx+1} done`); resolve(segmentPath); })
        //         .on('error', err => { console.error(`\n  ❌ Scene ${idx+1}:`, err.message); reject(err); })
        //         .run();
        // });
        // REPLACE the entire Promise block inside createSceneSegment with this:
        return new Promise((resolve, reject) => {
            const cmd = ffmpeg()
                .input(pattern)
                .inputOptions(['-framerate', String(fps)])   // ← remove -t from input
                .input(fitted.path)
                .outputOptions([
                    '-c:v', 'libx264',
                    '-pix_fmt', 'yuv420p',
                    ...
                    '-c:a', 'aac',
                    '-b:a', '192k',
                    '-t', String(duration),                  // ← only on output
                    '-y'                                     // ← overwrite if exists
                ]);

            // Only add -vf if captions is non-empty
            ...
            cmd
                ...
                .on('error', err => {
                    ...
                    console.error('FFmpeg stderr:\n', stderrLog);   // ← now you see the real error
                    reject(err);
                })
                .run();
        });
    ...
    // ── MERGE ─────────────────────────────────────────────────────────────────
    async mergeSegmentsWithTransitions(segPaths, scenes, outputName, td = 0.6) {
        const out = path.join(this.outputDir, outputName);
        if (segPaths.length === 1) { fs.copyFileSync(segPaths[0], out); return out; }

        const vf = [], af = []; let offset = 0;
        for (let i = 0; i < segPaths.length - 1; i++) {
            const a = i === 0 ? '[0:v]' : `[v${i - 1}]`, b = `[${i + 1}:v]`;
            const o = i === segPaths.length - 2 ? '[vout]' : `[v${i}]`;
            offset += (scenes[i].end - scenes[i].start) - td;
            vf.push(`${a}${b}xfade=transition=fade:duration=${td}:offset=${offset.toFixed(2)}${o}`);
        }
        af.push(`${segPaths.map((_, i) => `[${i}:a]`).join('')}concat=n=${segPaths.length}:v=0:a=1[aout]`);

        return new Promise((resolve, reject) => {
            const cmd = ffmpeg();
            segPaths.forEach(s => cmd.input(s));
            cmd.complexFilter([...vf, ...af])
                .outputOptions(['-map', '[vout]', '-map', '[aout]', '-c:v', 'libx264', '-c:a', 'aac', '-b:a', '192k', '-movflags', '+faststart'])
                .output(out)
                .on('progress', p => { if (p.percent) process.stdout.write(`\rMerging: ${p.percent.toFixed(1)}%`); })
                .on('end', () => { console.log(`\n✅ Final: ${out}`); resolve(out); })
                .on('error', err => { console.error('\nFallback concat:', err.message); this.mergeSegmentsSimple(segPaths, out).then(resolve).catch(reject); })
                .run();
        });
    }
    ...
    async mergeSegmentsSimple(segPaths, out) {
        ...
        return new Promise((resolve, reject) => {
            ffmpeg().input(concat).inputOptions(['-f', 'concat', '-safe', '0'])
                .outputOptions(['-c:v', 'libx264', '-c:a', 'aac', '-b:a', '192k', '-movflags', '+faststart'])
                .output(out).on('end', () => resolve(out)).on('error', reject).run();
        });
    }
    ...
    async generateVideoFromScenes(options = {}) {
        ...
        console.log('\n' + '═'.repeat(60));
        console.log('🎥  AI VIDEO GENERATOR — FREE (No API keys)');
        console.log('═'.repeat(60));
        IMAGE_PROVIDERS.forEach((p, i) => console.log(`  ${i + 1}. ${p.name}`));
        console.log('═'.repeat(60));

        this.cleanupTemporaryFiles();
        const segmentPaths = [];
        for (let i = 0; i < scenes.length; i++) {
            segmentPaths.push(await this.createSceneSegment(scenes[i], i, fps, stylePrompt, dimensions, captionStyle, aspectRatio));
            if (i < scenes.length - 1) await delay(500);
        }

        const finalPath = transition
            ? await this.mergeSegmentsWithTransitions(segmentPaths, scenes, outputName, transitionDuration)
            : await this.mergeSegmentsSimple(segmentPaths, path.join(this.outputDir, outputName));

        return { success: true, videoPath: finalPath, fileName: outputName, totalDuration, sceneCount: scenes.length, fps, aspectRatio, dimensions, captionType: 'voice-synced' };
    }
...
export async function generateVideoFromScenesHandler(generatedScript) {
    try {
        if (!Array.isArray(generatedScript) || !generatedScript.length) return { error: 'Scenes array required' };
        const result = await generator.generateVideoFromScenes({
            scenes: generatedScript, style: 'anime', fps: 8, aspectRatio: '9:16',
            captionStyle: 'bold', transition: true, transitionDuration: 0.6,
            outputName: `video_${Date.now()}.mp4`
        });
...
app.get('/', (req, res) => res.json({
    message: 'AI Video Generator v8.0 — Zero-Token Edition',
    providers: IMAGE_PROVIDERS.map((p, i) => `${i + 1}. ${p.name} (free, no key)`),
    note: '5x Pollinations models → Stable Horde → Craiyon → gradient placeholder'
}));

const PORT = 3000;
app.listen(PORT, () => {
    console.log(`\n${'═'.repeat(60)}`);
    console.log(`🚀  AI Video Generator v8.0 — Zero tokens required`);
    console.log(`${'═'.repeat(60)}`);
    IMAGE_PROVIDERS.forEach((p, i) => console.log(`  ${i + 1}. ${p.name}`));
    console.log(`${'═'.repeat(60)}\n`);
});